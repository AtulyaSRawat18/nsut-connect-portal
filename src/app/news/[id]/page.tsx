...
export default async function ResearchBriefDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="min-h-screen bg-background px-6 py-16 text-foreground"><article className="mx-auto max-w-4xl">
    <Link href="/feed" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-primary"><ArrowLeft className="h-4 w-4" /> Research feed</Link>
    <header className="mt-10 border-b border-outline pb-10">
      <div className="flex flex-wrap items-center gap-3 text-[10px] font-black uppercase tracking-widest">
        <span className="rounded-full bg-primary/10 px-3 py-1 text-primary">{item.category.replace("-", " & ")}</span>
        <span className="rounded-full bg-foreground/5 px-3 py-1 text-foreground/55">{getDepartmentLabel(item.department)}</span>
        <span className="text-foreground/45">Curated brief · {new Date(item.date).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</span>
      </div>
