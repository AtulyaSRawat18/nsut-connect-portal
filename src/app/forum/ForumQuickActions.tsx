    ...
    <div className="mt-5 border-t border-outline pt-4">
      ...
      {replyOpen && (
        <form onSubmit={submitReply} className="mt-4 rounded-xl bg-background p-4">
          ...
          <div className="mt-3 flex justify-end"><button disabled={pending === "reply" || content.trim().length < 2} className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white disabled:opacity-45">{pending === "reply" ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />} Post reply</button></div>
        </form>
      )}
