...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const limit = checkRateLimit(request, "forum:reply-vote", { limit: 40, windowMs: 60_000 });
  ...
  try {
    ...
    if (!z.string().uuid().safeParse(id).success) return authError(400, "INVALID_REPLY", "Invalid reply");
    const parsed = voteSchema.safeParse(await request.json().catch(() => null));
    if (!parsed.success) return authError(400, "INVALID_VOTE", "Vote must be up, down, or cleared");
    const supabase = await createClient();
    const current = await supabase.from("forum_reply_votes").select("value").eq("reply_id", id).eq("voter_id", identity.id).maybeSingle();
    ...
    if (requested === 0 || current.data?.value === requested) {
      const removed = await supabase.from("forum_reply_votes").delete().eq("reply_id", id).eq("voter_id", identity.id);
      if (removed.error) return authError(400, "VOTE_FAILED", removed.error.message);
    } else {
      const saved = await supabase.from("forum_reply_votes").upsert(
        { reply_id: id, voter_id: identity.id, value: requested, updated_at: new Date().toISOString() },
        { onConflict: "reply_id,voter_id" },
      );
      if (saved.error) return authError(400, "VOTE_FAILED", saved.error.message);
    }
    const { data: reply } = await supabase.from("forum_replies").select("score").eq("id", id).single();
    revalidateTag("public-forum", "max");
    return NextResponse.json({ error: false, score: reply?.score || 0, vote: requested === 0 || current.data?.value === requested ? 0 : requested });
  } catch (error) {
