...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  if (!isSameOrigin(request)) return authError(403, "INVALID_ORIGIN", "Cross-origin collaboration reviews are not allowed");
  const limit = checkRateLimit(request, "faculty:collaboration-review", { limit: 40, windowMs: 60_000 });
  if (!limit.allowed) return authError(429, "RATE_LIMITED", "Too many review actions", { "Retry-After": String(limit.retryAfterSeconds) });
  try {
    await requirePortalIdentity({ roles: ["faculty"], permissions: ["collaboration.review"] });
    const { id } = await params;
    if (!z.string().uuid().safeParse(id).success) return authError(400, "INVALID_REQUEST", "Invalid collaboration request");
    const parsed = reviewSchema.safeParse(await request.json().catch(() => null));
    if (!parsed.success) return authError(400, "INVALID_REVIEW", parsed.error.issues[0]?.message || "Invalid review");
    const supabase = await createClient();
    const { error } = await supabase.rpc("review_faculty_collaboration_request", { target_request_id: id, new_status: parsed.data.status, review_note: parsed.data.note || null });
    ...
    return authError(503, "REVIEW_UNAVAILABLE", "Collaboration review is temporarily unavailable");
  }
}
