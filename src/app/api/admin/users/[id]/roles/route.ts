...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  if (!isSameOrigin(request)) {
    return authError(403, "INVALID_ORIGIN", "Cross-origin role management is not allowed");
  }
  const limit = checkRateLimit(request, "admin:roles", { limit: 30, windowMs: 60_000 });
  if (!limit.allowed) {
    return authError(429, "RATE_LIMITED", "Too many role changes", { "Retry-After": String(limit.retryAfterSeconds) });
  }

  try {
    await requirePortalIdentity({ permissions: ["role.manage"] });
    ...
    const { error } = await supabase.rpc("assign_portal_role", {
      target_id: id,
