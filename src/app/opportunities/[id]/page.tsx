...
export default async function OpportunityDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="min-h-screen bg-background px-6 py-16 text-foreground"><article className="mx-auto max-w-4xl">
    <Link href="/opportunities" className="inline-flex items-center gap-2 text-xs font-black uppercase tracking-widest text-primary"><ArrowLeft className="h-4 w-4" /> Opportunities</Link>
    <header className="mt-10 border-b border-outline pb-10">
      <div className="flex flex-wrap items-center gap-3 text-[10px] font-black uppercase tracking-widest">
        <span className="rounded-full bg-primary/10 px-3 py-1 text-primary">{item.type}</span>
        <span className="rounded-full bg-foreground/5 px-3 py-1 text-foreground/55">{getDepartmentLabel(item.department)}</span>
        {item.deadline && <span className="flex items-center gap-2 text-foreground/45"><CalendarDays className="h-4 w-4" /> Review deadline {new Date(item.deadline).toLocaleDateString("en-IN", { day: "numeric", month: "long", year: "numeric" })}</span>}
      </div>
      <h1 className="mt-6 text-4xl font-black leading-tight tracking-tight md:text-6xl">{item.title}</h1><p className="mt-6 text-xl leading-relaxed text-foreground/65">{item.summary}</p>
    </header>
