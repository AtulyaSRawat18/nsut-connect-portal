...
function records() {
  ...
  const announcements = showcase.news.map((item, i) => ({ id: item.id, title: item.title, content: item.summary, source_url: item.sourceUrl, author_id: faculty[i].id, category: item.category, created_at: item.date }));
  const highlights = showcase.opportunities.map((item, i) => ({ id: item.id, title: item.title, description: item.summary, type: item.type, link_url: item.sourceUrl, deadline: item.deadline, created_at: ago(5 - i * 2) }));
  const reports = [
